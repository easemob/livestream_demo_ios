                    ...
                    completion:(void (^)(EMChatroom *aChatroom, EMError *aError))aCompletion
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [[EMClient sharedClient].roomManager joinChatroom:aChatroomId completion:aCompletion];
...
{
    NSString *accessToken = [EMClient sharedClient].accessUserToken;
...
{
    NSArray<NSString*> * array = [Appkey componentsSeparatedByString:@"#"];
    if (array.count < 2) {
        return;
    }
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/%@/%@/app/chat/user/register",AppServerHost,array[0],array[1]]];
    NSMutableURLRequest *request = [NSMutableURLRequest
                                                requestWithURL:url];
    request.HTTPMethod = @"POST";
                ...
                completion:(void (^)(NSInteger statusCode, NSString *response))aCompletionBlock
{
    ...
    NSURL *loginURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@/%@/%@/app/chat/user/login",AppServerHost,array[0],array[1]]];
    NSLog(@"%s loginURL:%@",__func__,loginURL.absoluteString);
...
{
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/inside/app/sms/send/%@",AppServerHost,phone]];
    NSMutableDictionary *headerDict = [[NSMutableDictionary alloc] init];
    [headerDict setObject:@"application/json" forKey:@"Content-Type"];
                       ...
                       completion:(void (^)(NSString * _Nullable response))aCompletionBlock
{
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/inside/app/user/login/V2?appkey=%@",AppServerHost,[appkey stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
    NSMutableURLRequest *request = [NSMutableURLRequest
                                                requestWithURL:url];
    request.HTTPMethod = @"POST";
